...
| Function URL auth | AWS_IAM (+SigV4 curl gymnastics) · **NONE + written justification + same-sitting teardown** | NONE, justified in writing | it's a lab endpoint alive for one sitting, receiving only telemetry-shaped JSON, writing only to its own logs. The build discovered **neither cdk-nag pack has a Function-URL-auth rule** (H12's either/or), so the justification lives as a stack comment + NOTES record — and "the guardrail you expected doesn't exist" became the sharper security lesson. IAM variant documented in the stack file for anyone keeping it up |
