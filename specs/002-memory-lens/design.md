...
- `unsafe impl GlobalAlloc for MemLens<A>` forwards to the inner allocator, then
  records `{op, addr, size, align, seq}` with `seq` from a global
  `AtomicU64::fetch_add` (monotonic ordering for R1/R4 without a global lock).
- **Reentrancy guard (the key trick):** recording an event itself allocates
  ...
  itself. This is also the teaching moment for `thread_local!` and why
  `GlobalAlloc` must be reentrancy-aware.
- **Writer:** a `Mutex<BufWriter<File>>` (path from `MEMLENS_TRACE` env, default
  `datalake/raw-local/traces/<name>-<pid>.jsonl`). Simple over clever: correctness
  first, overhead measured honestly (R14a). Flush on `Drop` of a `LensSession`
  guard and via an `atexit`-style hook.
...
- **Feature gating (R6):** everything is behind `#[cfg(feature = "memlens")]`; with
  the feature off, `MemLens<A>` is a zero-field passthrough whose methods inline to
  the inner allocator (verified by symbol-absence check, task-level).
...
realloc | dealloc | scope_enter | scope_exit | marker | loss | meta}`. Payload per
op: `{op, addr: "0x…", size, align, seq, label?}`. A `memlens.meta` header event
records program name, pid, start time, memlens version. Schema registered as
`datalake/schema/memlens.v1.json`. Addresses are recorded as opaque strings —
useful for matching, meaningless across runs (ASLR), and stated so (R15 honesty).
...
- `LiveSet` = fold of events `1..=t`: alloc inserts, dealloc removes, realloc
  removes+inserts; scope labels attach to allocations whose events fall inside the
  scope's enter/exit window.
...
- **Performance (R13):** canvas rendering (no DOM per event); prefix-sum arrays
  precomputed once at load so any scrub position is O(log n) lookup + one redraw;
  100k events ≪ 100 ms per step on any modern laptop.
...
- `thread_local!` + reentrancy — why the tracer must not trace itself.
- `Drop` guards in `lens_scope!` — deterministic cleanup even on early return/panic (L4).
...
- `AtomicU64` ordering for `seq` — first taste of `Sync` without a full async unit.
...
- **[O]** R9–R13, R14a, R15: learner session — run a prepared exercise, scrub the
  trace, record observations + measured overhead in `evidence.md`.
